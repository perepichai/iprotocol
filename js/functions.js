...
window.addEventListener('load', function () {
    ...
    //}
    for (y = 0; y < storage.length; y++) {
        document.querySelector('#' + storage[y]).addEventListener('click', function (eventObj) {
            var fieldForAutoFill = eventObj.target.id;//id обьекта события
            var node = document.querySelector(".dovidnyk");
            node.innerHTML = '';
            for (var i = 0; i < data[this.id].length; i++) {
                var div = document.createElement("div");
                ...
                p.addEventListener('dblclick', function() {
                    ...
                    var sk = document.querySelector('#' +fieldForAutoFill);
